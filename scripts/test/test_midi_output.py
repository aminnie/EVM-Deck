...
def test_midi_output(port_name=None):
    ...
    try:
        ...
        # Test 0: Send MIDI SysEx message (Ketron Start/Stop command) at the beginning
        print("Test 0: Sending MIDI SysEx Message (Ketron Start/Stop) - BEGIN")
        print("-" * 70)
        ...
        print()
        time.sleep(5)  # Wait 5 seconds before continuing with other tests
        
        # Send Start/Stop again after 5 seconds
        print("Sending Start/Stop again after 5 seconds...")
        print("-" * 70)
        # Send Start/Stop ON message
        sysex_on = [0x26, 0x79, 0x03, 0x12, 0x7F]  # Manufacturer (2 bytes), Type, Start/Stop, ON
        print(f"Sending Start/Stop ON: F0 {' '.join([hex(b) for b in sysex_on])} F7")
